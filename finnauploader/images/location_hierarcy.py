from images.models import FintoYsoLabel
...
# Split finto placename to crude hierarcy
def convert_subject_places_to_hierarcy(subject_places):
    ...
    for missing_keyword in missing_keywords:
        ...
        if subject_place_from_sparql:
            ...
            exit(1)
    validate_location_hierarcy(row)
    return row


def validate_location_hierarcy(row):
...
def is_luonto(place, wikidata_types, mml_types, other_types):
    luonto_qids = ['Q355304', 'Q106589819', 'Q39594', 'Q9258947',
...
def get_best_location_ids(row, allow_alt):
    ...
    best_ids = list(best_ids)
    if len(best_ids) > 1:
        wikidata_id = get_best_location_id(best_ids)
        if wikidata_id:
            best_ids = [wikidata_id]
    return best_ids


def get_best_location_id(wikidata_ids):
    print(wikidata_ids)
    ...
    return get_best_location_id(new_wikidata_ids)
